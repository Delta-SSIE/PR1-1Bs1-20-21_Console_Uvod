...
namespace _067_Minesweeper_enum
{
    enum StavPole { Neodkryto, Odkryto, Oznaceno, Bum }
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            StavPole[,] mapaHrace = new StavPole[vyska, sirka];
            ...
            while (!jeKonec)
            {
                ...
                switch (stisknuto.Key)
                {
                    ...
                    case ConsoleKey.Spacebar: //oznac/odznac
                        if (mapaHrace[kurzorY, kurzorX] == StavPole.Neodkryto)
                            mapaHrace[kurzorY, kurzorX] = StavPole.Oznaceno;
                        else if (mapaHrace[kurzorY, kurzorX] == StavPole.Oznaceno)
                            mapaHrace[kurzorY, kurzorX] = StavPole.Neodkryto;
                        break;

                    case ConsoleKey.Enter: //odkryj
                        if (mapaHrace[kurzorY, kurzorX] == StavPole.Oznaceno)
                            break;
                        bool jeMina = mapaMin[kurzorY, kurzorX];
                        if (jeMina)
                        {
                            mapaHrace[kurzorY, kurzorX] = StavPole.Bum;
                            vybuchnul = true;
                        }
                        else
                        {
                            mapaHrace[kurzorY, kurzorX] = StavPole.Odkryto;
                        }
                        break;
                }
        ...
        static void KresliMapu(StavPole[,] mapaHrace, byte[,] mapaSousedu, int kurzorX = -1, int kurzorY = -1)
        {
            ...
            for (int y = 0; y < vyska; y++)
            {
                ...
                for (int x = 0; x < sirka; x++)
                {
                    ...
                    StavPole stav = mapaHrace[y, x];
                    string symbol;
                    if (stav == StavPole.Neodkryto)
                        symbol = znakNeodkryto;
                    else if (stav == StavPole.Odkryto) //odkryto - zobraz číslo
                    {
                        symbol = mapaSousedu[y, x].ToString();
                        if (symbol == "0")
                            symbol = znakPrazdno;
                    }
                    else if (stav == StavPole.Oznaceno)
                    {
                        symbol = znakPraporek;
                    }
                    else
        ...
        /// </summary>
        /// <param name="mapaHrace">2D pole se stavem hráčovy mapy</param>
        /// <returns>2-prvkové pole se součty (index 0: odkryto, index 1: označeno)</returns>
        static int[] SpoctiStatistiku(StavPole[,] mapaHrace)
        {
            int vyska = mapaHrace.GetLength(0);
            int sirka = mapaHrace.GetLength(1);
            ...
            for (int y = 0; y < vyska; y++)
            {
                for (int x = 0; x < sirka; x++)
                {
                    if (mapaHrace[y, x] == StavPole.Odkryto)
                        odkryto++;
                    else if (mapaHrace[y, x] == StavPole.Oznaceno)
                        oznaceno++;
                }
            }
